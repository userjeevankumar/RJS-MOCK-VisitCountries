...
class VisitCountries extends Component {
  ...
  changeVisitedList = id => {
    ...
    /*
     ...
     const {initialVisitedCountriesList} = this.state
    const updatedPlayList = initialVisitedCountriesList.filter(
      each => each.id !== id,
    )
   
    const updatedVisitedPlayList = initialVisitedCountriesList.filter(
      each => each.id === id,
    )

    this.setState(prevState => ({
      visitedList: [...prevState.visitedList, updatedVisitedPlayList],
    })) 

    this.setState({initialVisitedCountriesList: updatedPlayList}) */
  }
      ...
      <div className="main-container">
        ...
        <div>
          <ul className="countries-list-container">
            {initialVisitedCountriesList.map(eachItem => (
              <CountriesList
                changeVisitedList={this.changeVisitedList}
                key={eachItem.id}
                countries={eachItem}
